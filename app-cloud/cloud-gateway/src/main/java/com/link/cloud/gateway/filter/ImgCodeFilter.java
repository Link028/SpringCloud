...
import com.link.cloud.gateway.ValidateCodeException;
...
public class ImgCodeFilter extends AbstractGatewayFilterFactory<ImgCodeFilter.Config>
{
    ...
    public GatewayFilter apply(Config config)
    {
        return (exchange, chain) -> {
            ...
            // 不是登录请求，直接向下执行
            if (!StringUtils.containsIgnoreCase(uri.getPath(), Constants.LOGIN_URL))
            {
            ...
            catch (Exception e)
            {
                ...
                String msg = JSON.toJSONString(RestMessage.error(e.getMessage()));
